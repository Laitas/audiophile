...
const Product = ({product}) => {
    ...
    return (
        <div className="md:even:flex-row-reverse flex md:flex-row flex-col md:my-12 my-6 items-center text-center">
            <img className='rounded flex md:w-1/2 max-h-[560px]' src={categoryImage.desktop} alt={name} />
            <div className="md:w-1/2 md:flex flex-col mx-4 md:mx-8">
            {product.new && 
            <h2 className='text-orange mt-4 uppercase tracking-[0.75em]'>New product</h2>
            }
            <h1 className='text-4xl font-bold uppercase my-4'>{name}</h1>
